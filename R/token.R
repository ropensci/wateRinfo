...
#'
#' @aliases token print.token show.token expires.in expires.in.token is.expired is.expired.token
#'
#' @param client base64 encoded client containing the client id and client secret, seperated by :
#' @param client_id client id string
#' @param client_secret client secret string
#' @param token_url url to get the token from
#' @param token a token object
#'
#' @details Either client or client_id and client_secret need to be passed as arguments.
#' If provided, client is always used. Tokens remain valid for 24 hours, after which a fresh one must be acquired.
#' To limit load on the server, token objects should be reused as much as possible until expiration in stead of creating fresh ones for each call.
#' The client_id and client_secret provided in the examples are for test purposes,
#' get your very own client via \email{hydrometrie@@waterinfo.be}!
#'
#' @return An object of class token containing the token string with the token_url, token type and moment of expiration as attributes.
#' 
#' @examples
...
get_token <- function(client=NULL,
  ...
  if(is.null(client)){
    if(!is.null(client_id) & !is.null(client_secret)){
      client <- openssl::base64_encode(paste(client_id,client_secret, sep = ":"))
    } else {
      stop('either client or client_id and client_secret must be provided')
    }
  } else {
    if(!is.null(client_id) | !is.null(client_secret)){
      warning('both client and client_id and/or client_secret provided, using client')
    }
  }
  POST_call <- httr::POST(url=token_url,
                    config=httr::add_headers('Authorization' = paste0('Basic ',client),
                                       'scope' = 'none',
                                       'Content-Type' = 'application/x-www-form-urlencoded;charset=UTF-8'),
                    body='grant_type=client_credentials')
  if(POST_call$status_code==201){
    out <- token(value=httr::content(POST_call)$access_token,
                 url=token_url,
                 type=httr::content(POST_call)$token_type,
                 expires=Sys.time()+httr::content(POST_call)$expires_in)
    return(out)
  } else {
    stop(paste0('no token received, ',POST_call$url,' returned status code: ',POST_call$status_code))
  }
}
...
print.token <- function(x,...){
  cat('Token:\n',x,'\n\nAttributes:\n url: ',attr(x,'url'),'\n type: ',attr(x,'type'),'\n expires: ',format(attr(x,'expires'),'%F %T %Z'),sep='')
  invisible(x)
}
